...
import com.mehdigm.cimastream4.CommonActivity.isInPIPMode
import com.mehdigm.cimastream4.CommonActivity.screenWidth
...
import com.mehdigm.cimastream4.ui.player.live.LivePreviewTimeBar
...
import com.mehdigm.cimastream4.utils.AppContextUtils
import com.mehdigm.cimastream4.utils.AppContextUtils.requestLocalAudioFocus
...
import com.mehdigm.cimastream4.utils.UIHelper.showSystemUI
import com.mehdigm.cimastream4.utils.UserPreferenceDelegate
...
class PlayerView @JvmOverloads constructor(
    ...
    fun initialize() {
        ...
        if (player is CIMA4IPlayer) {
            ...
            if (progressBar != null && previewImageView != null && previewFrameLayout != null && !isLayout(TV)) {
                ...
                progressBar.addOnScrubListener(object : PreviewBar.OnScrubListener {
                    override fun onScrubStart(previewBar: PreviewBar?) {
                        val cima4 = player as? CIMA4IPlayer ?: return
                        val hasPreview = cima4.hasPreview()
                        progressBar.isPreviewEnabled = hasPreview
                        resume = cima4.getIsPlaying()
                        if (resume) cima4.handleEvent(CSPlayerEvent.Pause, PlayerEventSource.Player)
                        // No clashing UI
                    ...
                    override fun onScrubStop(previewBar: PreviewBar?) {
                        val cima4 = player as? CIMA4IPlayer ?: return
                        if (resume) cima4.handleEvent(CSPlayerEvent.Play, PlayerEventSource.Player)
                ...
                progressBar.setPreviewLoader { currentPosition, max ->
                    val cima4 = player as? CIMA4IPlayer ?: return@setPreviewLoader
                    val bitmap = cima4.getPreview(currentPosition.toFloat().div(max.toFloat()))
            ...
            (player as? CIMA4IPlayer)?.initSubtitles(subView, subtitleHolder, CustomDecoder.style)
            (player as? CIMA4IPlayer)?.let {
