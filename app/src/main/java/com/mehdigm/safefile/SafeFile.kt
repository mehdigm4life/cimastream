...
interface SafeFile {
    ...
    fun findFile(displayName: String?, ignoreCase: Boolean = false): SafeFile? {
        return try {
            findFileOrThrow(displayName, ignoreCase)
        } catch (e: FileNotFoundException) {
            // "File not found" is the expected result when the file simply does not exist.
            // This used to log a full "No file found" error stack on every lookup miss.
            null
        } catch (t: Throwable) {
            logError(t)
            null
